...
class EnvironmentDetector:
    ...
    def detect_git_tools(self) -> Dict[str, Any]:
        ...
        if tools["gh"]:
            try:
                result = subprocess.run(
                    ["gh", "--version"], capture_output=True, text=True, check=True, timeout=5
                )
    ...
    def _generate_recommendations(self) -> None:
        ...
        if not git_tools.get("git"):
            self.recommendations.append(
                Recommendation(
                    priority="HIGH", message="Git is not installed", action="install_git"
                )
            )
        ...
        if not git_tools.get("gh"):
            self.recommendations.append(
                Recommendation(
                    priority="HIGH", message="GitHub CLI is not installed", action="install_gh"
                )
            )
        elif not git_tools.get("gh_authenticated"):
            self.recommendations.append(
                Recommendation(
                    priority="MEDIUM", message="GitHub CLI is not authenticated", action="auth_gh"
                )
            )
